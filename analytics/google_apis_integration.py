...
class GoogleAPIsIntegration:
    ...
    def get_top_pages_gsc(self, date_range_days=30, limit=100, site_name='moodmark', start_date=None, end_date=None):
        """
        GSCから上位ページデータを取得
        ...
        if start_date and end_date:
            ...
            logger.info(f"GSCページデータ取得: サイト={site_name}, 期間={start_date} ～ {end_date}")
            response = self.gsc_service.searchanalytics().query(
                siteUrl=gsc_site_url,
                body=request
            ).execute()
            ...
            if 'rows' in response:
                for row in response['rows']:
                    row_data = {}
                    # ディメンション値
                    for i, dimension in enumerate(['page']):
                        if i < len(row.get('keys', [])):
                            row_data[dimension] = row['keys'][i]
                    # メトリクス値
        ...
        # limitで絞る
        if len(page_stats) > limit:
            page_stats = page_stats.head(limit)
