...
#### Render 無料プランとメモリ（ログアウト事象）

無料 Web サービスは **メモリ 512MB** です。記事削除のたびにページ全体が再実行され、大きな `last_snapshot`（JSONB）を pandas で展開するとメモリが積み上がり、**インスタンス再起動**（Render Logs の `Instance ... restarted`）→ Streamlit の `session_state` 消失 → **ログイン画面に戻る**、という流れが起き得ます。

- **確認**: Render Dashboard → 対象 Web サービス → **Metrics** で `memory_usage` と再起動時刻の相関を確認
- **対策**: 本番で記事数・SKU数が多い場合は有料プラン（メモリ増）、または JSON バックアップでの一括編集で操作回数を減らす
- **2026-06-24 の事例**: 約2分間に多数の rerun の直後 `Instance restarted` が記録（削除操作と同時間帯）
