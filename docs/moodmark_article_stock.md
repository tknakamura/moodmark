...
#### 再起動とログイン維持（ログアウト事象の本当の原因）

「記事削除中にログイン画面へ戻る」事象は、**メモリ不足（OOM）ではなく、Render インスタンスの再起動**が原因でした。Streamlit のログイン状態は `st.session_state`（サーバーのメモリ内）にしか無いため、再起動が起きると操作内容に関係なくログイン画面へ戻ります。

2026-06-24 の調査（Render Metrics / Logs）で判明したこと:

- **OOM ではない**: 再起動前後のメモリはピークでも約 250〜350MB / 上限 536MB。
- **CPU 超過でもない**: ピーク 0.14 / 上限 0.5 コア。
- **アプリのクラッシュでもない**: 再起動直前に Python の例外・traceback は記録なし。
- **頻度**: 定常運用中（デプロイなし）は 8 時間以上 0 回。再起動はデプロイ直後の時間帯に集中。→ デプロイに伴うインスタンス入れ替えが主因。

**対策（実装済み）**: ログインを**署名付き Cookie で永続化**しました（[csv_to_html_dashboard.py](../csv_to_html_dashboard.py)）。再起動が起きても Cookie が有効なら自動で再ログインされ、ログイン画面には戻りません。
...
**補足（メモリの余裕）**: 「結果の表示」タブで大きなスナップショットを描画するとメモリが一時的に約 320〜350MB まで上がります。上限 536MB に対し余裕は約 190MB で、**複数人が同時に重いタブを開くと 512MB を超える恐れ**があります。同時利用が多い場合は有料プラン（メモリ増）を検討してください。
