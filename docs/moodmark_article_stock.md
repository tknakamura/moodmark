...
- **登録一覧の日時列**: 最終スナップショットの `cache_meta` に基づき、**記事ページ取得(UTC)**（`articles[記事URL].fetched_at`＝記事 HTML から掲載商品 URL を取り直した時刻）と **掲載商品在庫の最新(UTC)**（当該記事の掲載リストに含まれる商品の `products[商品URL].checked_at` の最大）を表示します。在庫チェック未実行やメタ欠落時は空欄です。
