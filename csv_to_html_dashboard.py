...
# ログイン永続化用のCookie設定
AUTH_COOKIE_NAME = "moodmark_auth"
AUTH_TTL_SECONDS = 24 * 60 * 60  # 1日（再起動をまたいでもこの期間はログイン維持）


def _auth_secret() -> str:
    """トークン署名用の秘密鍵。環境変数優先、無ければ users.json から安定的に導出。

    Render の再起動をまたいでもトークンを有効に保つため、サーバー固有のランダム値ではなく
    リポジトリに含まれる users.json のハッシュ群から決定的に算出する（再起動後も同じ鍵）。
    """
    env = os.environ.get("AUTH_COOKIE_SECRET", "").strip()
    if env:
        return env
    users = load_users()
    base = "|".join(sorted((u.get("password_hash") or "") for u in users)) or "moodmark-default"
    return hashlib.sha256(("moodmark-auth::" + base).encode("utf-8")).hexdigest()


def _sign_auth_token(username: str, expires_at: float) -> str:
    msg = f"{username}:{int(expires_at)}"
    sig = hmac.new(
        _auth_secret().encode("utf-8"), msg.encode("utf-8"), hashlib.sha256
    ).hexdigest()
    return base64.urlsafe_b64encode(f"{msg}:{sig}".encode("utf-8")).decode("utf-8")


def _verify_auth_token(token):
    """トークンが正当（署名一致・未失効）なら username を返す。無効なら None。"""
    if not token:
        return None
    try:
        raw = base64.urlsafe_b64decode(token.encode("utf-8")).decode("utf-8")
        username, exp_str, sig = raw.rsplit(":", 2)
...
def _get_auth_cookie_manager():
    """スクリプト実行ごとに1つだけ CookieManager を生成し session_state に保持。"""
    mgr = stx.CookieManager(key="moodmark_auth_cm")
...
def require_dashboard_login() -> None:
    ...
    サーバー再起動で session_state が消えても、ブラウザの署名付き Cookie が有効なら
    自動的にログイン状態を復元する。
    """
    check_authentication()
    cookie_manager = _get_auth_cookie_manager()
...
def logout() -> None:
    """セッションと Cookie を破棄してログアウト。"""
    cookie_manager = st.session_state.get("_auth_cookie_mgr") or _get_auth_cookie_manager()
    try:
    ...
    st.session_state.authenticated = False
    st.session_state.username = None
    # Cookie 削除のJSがブラウザに届く猶予を与えてから再実行
    time.sleep(0.3)
    st.rerun()
                ...
                for user in users:
                    if user['id'] == username:
                        ...
                        if verify_password(password, user['password_hash']):
                            st.session_state.authenticated = True
                            st.session_state.username = username
                            mgr = cookie_manager or st.session_state.get("_auth_cookie_mgr")
                            if mgr is not None:
                                expires_at = time.time() + AUTH_TTL_SECONDS
                                token = _sign_auth_token(username, expires_at)
                                try:
                                    mgr.set(
                                        AUTH_COOKIE_NAME,
                                        token,
                            ...
                            # Cookie 書き込みのJSがブラウザに届く猶予を与えてから再実行
                            time.sleep(0.3)
                            st.rerun()
                        else:
